...
class AgreementFrame(tk.Frame):
    ...
    def setup_ui(self):
        # 제목
        title_label = tk.Label(self, text="열쇠 키오스크 이용약관",
                              font=("맑은고딕", 20, "bold"))
        ...
        # 주의사항 텍스트
        notice_text = tk.Text(notice_frame, height=10, width=60, wrap=tk.WORD,
                             font=("맑은고딕", 16))
        ...
        # 동의 버튼
        agree_btn = tk.Button(button_frame, text="동의합니다",
                             command=self.app.show_main_screen,
                             width=20, height=3, bg="lightgreen",
                             font=("맑은고딕", 14, "bold"),
                             relief="raised", bd=3)
        agree_btn.pack(side=tk.LEFT, padx=10)

        # 비동의 버튼
        deny_btn = tk.Button(button_frame, text="동의하지 않습니다",
                            command=lambda: messagebox.showwarning("경고",
                                "위 내용에 동의하지 않으면 사용하실 수 없습니다"),
                            width=20, height=3, bg="lightcoral",
                            font=("맑은고딕", 14, "bold"),
                            relief="raised", bd=3)
